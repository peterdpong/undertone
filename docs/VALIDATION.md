...
- Audio kernel tests with AddressSanitizer and UndefinedBehaviorSanitizer: linear gain, interleaved and planar stereo, physical-input exclusion, mono downmix, mute, sample-rate-aware volume smoothing, short input, extra output channels, null buffers, and nonfinite gain.
- Volume transitions use a 30 ms exponential time constant. Regression checks verify the same response at 44.1/48/96/192 kHz, identical results across callback boundaries, and exact settled gain/mute.
...
- Screen-sharing investigation: one-second steady-tone tests at 100%, 200%, and 400%, both below and above the limiter threshold, preserve settled periodicity and match across variable callback boundaries. These offline tests cover the render kernel only; they do not exercise tap resampling, simultaneous capture, FaceTime encoding, microphone pickup, or the viewer's playback.
- Local boost quality: replaced the instantaneous peak shaper after a user reported local graininess around 140%. Steady-tone checks at 140%/400%, 40/200/1000/6000 Hz, and 44.1/48/96/192 kHz measure less than 0.001% residual after fitting a linear gain to the settled waveform. Added sudden-peak bounds, stereo linking, timed hold/recovery, reset, and invalid sample-rate checks. These offline measurements exclude the initial limiter attack and do not establish end-to-end speaker quality or resolve FaceTime sharing.
...
- Output-only controls: removed input discovery, selection, gain, and observers from the app model and device controls. Presets now capture, display, compare, and apply output settings only. Legacy input fields are ignored on load and omitted on subsequent saves; compatibility regression passes. Physical-input exclusion in the playback engine remains in place.
...
7. Connect a microphone and a full-duplex USB/Bluetooth device. Verify output routing and confirm microphone input never plays through the output.
...
13. Retest the user's locally grainy source around 140% with the new limiter, then compare 100%, 200%, and 400% using speakers and headphones. Test sustained tones, music, speech, and sharp transients separately. This local quality check is independent of the remote FaceTime failure above.
